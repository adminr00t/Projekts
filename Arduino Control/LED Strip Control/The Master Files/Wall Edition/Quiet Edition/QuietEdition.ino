...
#pragma region maths
    sensorvalue = analogRead(DA); //Read the analog value
    EMA_S = (EMA_a * sensorvalue);
    int overtrsh = (EMA_S - trsh) + 25;  //Divides by 2
    int overtrsh2 = (EMA_S - trsh) + 25; //Divides by 2
    int avOvertrsh = (overtrsh + overtrsh2) >> 1;
    // FastLED.setBrightness(avOvertrsh);
    // analogWrite(OutputPin, sensorvalue);
#pragma endregion
...
#pragma region Colors for Strip2
    for (int i = 0; i < overtrsh2 && i < 42; i++)
    {
        strip2[i] = CRGB::Fuchsia;
        fadeallVibes2();
    }
...
void addGlitter(fract16 chanceOfGlitter)
{
...
void synthRainbow()
{
    ...
    fill_gradient_RGB(strip1, 51, CRGB::Orchid, NUM_LEDS, CRGB::DarkRed);

    //strip2
    fill_gradient_RGB(strip2, 0, CRGB::LightSeaGreen, 48, CRGB::Fuchsia);
    fill_gradient_RGB(strip2, 49, CRGB::Orchid, NUM_LEDS2, CRGB::DarkRed);
}
...
void ColorfulFire(CRGBPalette16 gPal)
{
    ...
    for (int i = 0; i < 62; i++)
    {
        heat2[i] = qsub8(heat[i], random8(0, ((COOLING * 10) / 64) + 2));
    }

    // Step 2.  Heat from each cell drifts 'up' and diffuses a little
    for (int k1 = 64 - 1; k1 >= 2; k1--)
    {
        heat[k1] = (heat[k1 - 1] + heat[k1 - 2] + heat[k1 - 2]) / 3;
    }
    for (int k2 = 62 - 1; k2 >= 2; k2--)
    {
...
void MeteorRight(int red, int green, int blue, int size, int tailDecay, boolean randomDecay, int speedDelay)
{
    ...
    for (int i = 0; i < FireLEDS2 + FireLEDS2; i++)
    {
        // fade brightness all LEDs one step
        for (int j = 0; j < FireLEDS2; j++)
        {
            if ((!randomDecay) || (random(10) > 5))
            {
