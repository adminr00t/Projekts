...
#Part 1 - Asks user for drive letter, verifies drive and folders exist. Creates missing folders
    ...
    printf "Enter drive letter for backup location:\n"
    read DRLT

    #debugging
    # pwd 

    #checks if drive directory exists, if it does, cd is changed. Else program exits
    if [ -d /mnt/$DRLT ]
    then 
        cd /mnt/$DRLT/
    else
        echo "directory /mnt/$DRLT does not exist"
        exit 1
    fi
